...
RESULT_DIR = PROJECT_ROOT / "results" / "smoke_tests" / "permutation_diagnostics"
...
def build_report(aggregate_rows: list[dict], negative_rows: list[dict], external_rows: list[dict]) -> str:
    lines = [
        "# permutation diagnostics smoke test",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run permutation diagnostics checks for FAA wildlife component screening.")
    ...
    result_dir = PROJECT_ROOT / "results" / "experiments" / "permutation_diagnostics" if args.full else RESULT_DIR
    ...
    write_csv(result_dir / "screening_diagnostics_yearly.csv", recent_rows)
    write_csv(result_dir / "screening_diagnostics_aggregate.csv", recent_aggregate)
    ...
    (result_dir / "permutation_diagnostics_report.md").write_text(report, encoding="utf-8")
    print(report)
