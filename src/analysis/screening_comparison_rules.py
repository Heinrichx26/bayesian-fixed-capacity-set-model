...
RESULT_DIR = PROJECT_ROOT / "results" / "screening_comparison" / "faa_wildlife"
...
def build_report(aggregate: list[dict], counter_summary: list[dict], added_examples: list[dict]) -> str:
    ...
    lines = [
        "# FAA wildlife strike component screening rolling validation",
        "",
        "## Inspection setting",
        "",
...
def main() -> None:
    ...
    write_csv(RESULT_DIR / "screening_comparison_yearly_metrics.csv", yearly)
    write_csv(RESULT_DIR / "screening_comparison_aggregate_metrics.csv", aggregate)
    write_csv(RESULT_DIR / "screening_comparison_counterfactual_added_records.csv", added)
    write_csv(RESULT_DIR / "screening_comparison_counterfactual_summary.csv", counter_summary)

    report = build_report(aggregate, counter_summary, added)
    (RESULT_DIR / "screening_comparison_report.md").write_text(report, encoding="utf-8")
    print(report)
